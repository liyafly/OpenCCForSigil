...
runs the Linux differential test, builds the Fat Plugin ZIP, and validates its
licenses/notices. It uploads one final installable plugin ZIP. A payload marked
`skipped-cross-platform` cannot enter the final artifact.

## Release asset contract

For future workflow runs, `build-fat-plugin` uploads one Actions artifact named
`OpenCCForSigil-fat-plugin-${{ github.sha }}` containing
`dist/OpenCCForSigil_${{ github.sha }}.zip`. The tagged `publish-release` job
downloads that exact commit-named file, verifies `plugin.xml` against the tag,
renames it to `release-artifacts/OpenCCForSigil_<version>.zip`, and passes that
version-named file as the release's one uploaded product asset. The archive has
the single top-level `OpenCCForSigil/` directory required by Sigil and contains
the verified cross-platform payloads.
