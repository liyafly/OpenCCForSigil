...
> 5. 在不改变 EPUB source-span、preview/transaction、rules、logs、golden 和 regional explicitness 设计的前提下，增加官方 `opencc-jieba` native plugin payload；它是按 manifest 检测的高级 config family，不是 Python Jieba 重写，也不是新的 production backend。
---
...
## 4.4 官方 native `opencc-jieba`：可检测的高级能力

OpenCC upstream 1.4.x 的 `plugins/jieba` 提供官方 C++ native segmentation
plugin `opencc-jieba`，以及 plugin-backed configs：
...
这些 config 仍然由同一个官方 Python Binding 调用：

```python
import opencc

converter = opencc.OpenCC("s2twp_jieba")
result = converter.convert(text)
```

`opencc-jieba` 不是 `opencc-py`，不是 Python Jieba 重写，也不是
OpenCCForSigil 自行实现的 longest-match 或词典算法。它是 pinned
BYVoid/OpenCC release 中的官方 native plugin；最终转换仍由官方 OpenCC
C++ Core 执行。OpenCC upstream 当前将该 plugin 体系标为 optional/
experimental，因此本项目必须 pin tag/commit、记录 plugin ABI/build
profile、library SHA-256 和全部 Jieba resource hashes，并在升级时重新执行
完整差分验证。

### 4.4.1 V1 高级选项边界

v1.4 允许在 UI 中出现一个**可检测的高级开关**，而不是任意 segmentation
dropdown：

```text
标准 config 下拉：s2t / s2tw / s2twp / …
高级开关：使用官方 native Jieba 分词插件
```

只有当当前 RuntimeSelector 已完成以下检查时，开关才能显示为可用：

1. selected payload 的 `native_plugins.opencc-jieba` manifest record 存在；
2. plugin upstream version/tag/commit 与 wheel/OpenCC payload 一致；
3. native library 路径在 selected payload 内，且 library hash 匹配；
4. 7 个 official plugin configs 和 Jieba resources 的 hashes 匹配；
5. 当前 `opencc.CONFIGS` 确实发现全部 plugin config；
6. Python Binding smoke test 与 official CLI differential test 已通过。

开关选中的是映射后的具体 config（例如 `s2t` → `s2t_jieba`），该 config
必须写入 `ConvertRequest`、`ConversionPlan`、preview、provenance、logging
与 report。没有对应 plugin config 的方向不得显示可用开关。缺少或校验失败
时必须 fail closed，只保留标准 OpenCC config，不得退回 `opencc-py`、system
plugin、system OpenCC、Python Jieba 或 subprocess converter。

`Profile.segmentation` 只允许 `"mmseg"` 或 `"jieba"`；它是已选 config 的
派生记录，不是独立的算法选择器。`jieba` 只有在具体 `*_jieba` config 和
manifest-approved plugin 同时存在时才合法。

### 4.4.2 Build/Release payload

PyPI `opencc` official wheel 通常不包含这个 optional plugin。因此每个正式
支持 payload 必须在 Build/Release 阶段由 pinned upstream source checkout
构建，而不是在用户运行时编译或下载：

```text
official OpenCC wheel payload
    + official OpenCC plugins/jieba source at same tag/commit
    + same-release OpenCC static core
    ↓
platform-specific opencc-jieba library
    + 7 official config files
    + cppjieba-derived Jieba dictionary resources
```

构建出的 plugin 只能放入其匹配的 payload：
...
实际文件名、OS、architecture 和 loader layout 必须由目标 runner 产物写入
manifest，不能用 macOS library 冒充 Windows/Linux。Windows 的 CMake
multi-config build 必须显式使用 `Release`，并将 plugin 放到 wheel 的
`opencc/clib/bin/plugins`；POSIX payload 使用 `opencc/clib/lib/opencc/plugins`。

每个 payload 的 plugin record 至少包括：
...
```json
{
  ...
  "upstream_commit": "...",
  "plugin_dir": "opencc/clib/lib/opencc/plugins",
  "library_path": "opencc/clib/lib/opencc/plugins/libopencc-jieba.dylib",
  "library_sha256": "...",
  "config_names": ["s2t_jieba", "s2tw_jieba", "s2twp_jieba", "s2hk_jieba", "s2hkp_jieba", "tw2sp_jieba", "hk2sp_jieba"],
  "resource_hashes": {},
  "resource_manifest_sha256": "...",
  "build_profile": "official OpenCC plugins/jieba against same-release static core"
}
```

`OPENCC_DATA_DIR` 与 `OPENCC_SEGMENTATION_PLUGIN_PATH` 只能由
RuntimeSelector 指向 selected payload；不得继承或搜索系统 plugin path。官方
OpenCC dictionaries/configs 和 Jieba resources 仍是 read-only；用户规则仍是
overlay，不得修改这些资源。
...
→ verify payload tree hash, native plugin/library/resource hashes and manifest compatibility
→ if a native plugin record exists, set `OPENCC_DATA_DIR` and exact
  `OPENCC_SEGMENTATION_PLUGIN_PATH` inside selected payload
...
- `OPENCC_SEGMENTATION_PLUGIN_PATH` 继承自用户环境或系统安装时不得继续使用；
  RuntimeSelector 必须先清除/覆盖它为当前 manifest-approved payload path；
- `*_jieba` config 的 native plugin、config 或 resource 任一校验失败时必须
  fail closed，不得把同名 config 交给系统 plugin 解析。
...
- `segmentation` 是已选 config 的派生值：标准 config 为 `"mmseg"`，官方
  `*_jieba` config 为 `"jieba"`；**UI 不提供任意 segmentation dropdown**，只在
...
```json
{
  ...
  "native_plugin_name": "opencc-jieba",
...
如果 release 要暴露高级 Jieba，matrix job 还必须从同一 pinned upstream
checkout 构建官方 `plugins/jieba`，将其 native library/config/resource 写入
该 payload，并运行 `differential_jieba_test.py`。Windows、macOS、Linux
必须分别生成自己的 DLL/dylib/so；禁止跨平台复制 native binary。
...
`tools/build_plugin.py` 与 `tools/build_opencc_jieba.py`：
...
5. 对每个目标 payload 构建同一 pinned release 的官方 `opencc-jieba` plugin，
   并校验 plugin ABI、config/resource hash、loader path 和 target runtime smoke；
6. 生成 `vendor/opencc/manifest.json`；
7. 运行 standard 与 native Jieba tests；
8. 检查 plugin.xml；
9. 校验 `THIRD_PARTY_NOTICES`、OpenCC license/authors、cppjieba MIT notice、
   无 `.pyc`/`__pycache__`/开发文件和 accidental build paths；
10. 解包自检；
11. 构建：
...
artifact；如果启用 Jieba，还必须通过 native Jieba CLI/Python Binding
differential。汇总 job 才能合并 manifest 并构建 Fat Plugin。仅完成 extraction
...
| mmseg | 保留 |
| Jieba | 作为官方 native `opencc-jieba` 高级 payload；仅 manifest 检测到并通过目标 runner 验证时启用，非独立 backend、非 Python 重写 |
| 大陆/台湾/香港 | 保留 |
| 繁体地区互转 | 保留 |
| quotes | 保留，独立模块 |
...
- 将 Jieba 做成任意可切换的 segmentation engine、Python fallback 或未验证的
  system plugin；v1.4 只接受 §4.4 的官方 native payload gate。
...
- 进一步扩展官方 native plugin 的 segment inspection/annotation 能力（不改变
  v1.4 的转换 config 选择）；
