...
> 基线日期：2026-09-03  
> 文档版本：**1.2（Native OpenCC 架构锁定版，2026-09-03）**  
> 建议插件暂定名：`OpenCCForSigil`  
> 插件类型：Sigil `edit` plugin  
> 核心目标：在 **不破坏 EPUB 结构与排版语义** 的前提下，提供可预览、可审计、可扩展、可测试的简繁体及地区词汇转换能力。

> **1.2 修订摘要**（详见 `REVISION_NOTES.md`）：
> 1. V1 唯一生产后端与 canonical oracle 统一为 **BYVoid/OpenCC native 1.4.2**；移除 `opencc-py` / `opencc-data` 运行时依赖。
> 2. Python 通过 `ctypes` 调用 OpenCC 官方稳定 C ABI（`opencc.h`）；按 OS/Arch 打包经过哈希验证的 `libopencc` 与官方 config/dictionary 数据。
> 3. native loader 只允许按 `vendor/MANIFEST.json` 的绝对路径加载，不使用系统 `opencc`、`find_library()` 或用户 PATH；移除旧 `sys.path` vendor 隔离规则。
> 4. 修正 XHTML 保真规范：`HTMLParser` 只作事件/状态辅助，插件自行维护绝对 source span；**只有 ConversionPlan 明确列出的文本/属性 span 可变化，所有未计划 source slice 必须逐字节保持**。
> 5. NCX / OPF 也改为 source-preserving patch + `lxml` 只读验证，避免 serializer 改写前缀、空元素和无关字节。
> 6. `s2t/tw2t/hk2t` 的 Legacy lang 不再自动标成 `zh-TW`；通用繁体没有地区语义时默认保持原语言标签，BCP47 建议 `zh-Hant`。
> 7. v1.1 的 `layered_diff/layer_chain` 重命名为 **`comparative_config_diff/comparison_configs`**：多个 config 仅用于结果分类，不代表 OpenCC 内部真实执行层；`rule_source` 始终记录实际选择的 config。
> 8. V1 UI 删除“分词策略”下拉；标准 config 的 mmseg 属 backend 固定行为。Jieba 仍移出 V1，但修正为 OpenCC upstream 已有 native `opencc-jieba` 插件，V2 再评估打包与回归成本。
> 9. 更新 CI、供应链、Self-Test、目录结构、Golden Oracle、Differential Test、Phase 1 与不可妥协项，使任何实现模型只有一条 native backend 路径。
---
...
## 4.1 V1 唯一后端：官方 Native OpenCC

V1 **只允许一个生产后端**：

- BYVoid/OpenCC **native 1.4.2**；
- 使用官方 `libopencc`；
- Python 通过标准库 `ctypes` 调用 OpenCC 官方稳定 C ABI；
- config 与 dictionary 均来自同一 pinned OpenCC upstream tag/commit；
- **不依赖任何 pure-Python OpenCC 重实现、独立 `opencc-data` 包、PyPI native Python extension 或用户系统安装的 OpenCC。**

Canonical oracle 也是同一 pinned BYVoid/OpenCC native release，但测试调用路径使用由同一 source tag 构建的官方 CLI `opencc`，与插件的 C ABI wrapper 形成独立调用路径（§29、§75）。

### 4.1.1 为什么使用 C ABI + `ctypes`

OpenCC 官方 `src/opencc.h` 将 C API 定义为面向 downstream/bindings 的稳定 ABI，核心接口包括：

```text
opencc_open
opencc_open_w          # Windows / MSVC 构建可用
opencc_convert_utf8
...
选择它而不是 native Python extension 的原因：

1. **避免 CPython ABI 绑定**：Sigil 升级 Python 时无需重新适配 `.pyd/.so` Python extension ABI；
2. **仍是官方 native 引擎**：转换、分词、config chain、dictionary semantics 全部由 `libopencc` 执行；
3. **无每段 subprocess 开销**：一个 session 内复用 converter handle；
4. **跨平台边界清晰**：Python 代码完全一致，仅 native library 文件按 platform triple 选择；
5. **更容易验证供应链**：实际加载 library 的 SHA-256 必须与 `vendor/MANIFEST.json` 一致。

插件不得自行实现 C++ conversion logic，也不得将官方 CLI 逐 TextTarget 启动作为普通转换路径。

### 4.1.2 已核验事实（2026-09-03，作为本文基线）

| 项目 | 事实 / 规范结论 |
|---|---|
| Canonical source | `BYVoid/OpenCC` tag `ver.1.4.2` |
| C ABI | `opencc.h` 暴露 `opencc_open/opencc_convert_utf8/opencc_convert_utf8_free/opencc_close/opencc_error`；官方源码将其定义为 Stable C ABI |
| config/resource | `opencc_open()` 可接收 config path；OpenCC 会把 config 所在目录加入资源搜索路径，因此 bundled config 与 `.ocd2` 可共同放入受控 data directory |
| 标准 config | V1 allowlist：`s2t t2s s2tw tw2s s2twp tw2sp s2hk hk2s s2hkp hk2sp t2tw tw2t t2hk hk2t t2jp jp2t` |
| normalization | 官方 1.4.x config 可含 normalization 阶段，它属于 canonical output，不得由插件绕过 |
| tofu-risk | OpenCC `ConfigLoadOptions` 默认 `includeTofuRiskDictionaries=true`；稳定 C API `opencc_open()` 走默认 options，V1 因此固定为 include，不提供 UI 开关 |
| error API | 官方注明 `opencc_error()` 是 C API 中唯一非线程安全函数；V1 backend 对 error retrieval 使用锁，并默认单 worker 转换 |
| Jieba | OpenCC upstream 已存在 native external segmentation plugin `opencc-jieba` 与相应 config；V1 不打包它，见 §4.4 |

这些事实在 OpenCC 升级时必须按 §44 重新核验，不得从旧版本推断。
...
```text
OpenCC upstream tag
OpenCC upstream commit
native build recipe revision
每个平台 native library SHA-256
所有 bundled config / dictionary SHA-256
```

`vendor/MANIFEST.json` 至少：
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      "library": "native/windows-x86_64/opencc.dll",
      "sha256": "...",
      "runtime_dependencies": []
    },
    "macos-arm64": {
      "library": "native/macos-arm64/libopencc.dylib",
      "sha256": "...",
      "runtime_dependencies": []
    }
  },
...
不得只显示“最新版 OpenCC”，不得以文件名推断版本。

## 4.3 Native bundle 与平台策略
...
macos-arm64
linux-x86_64
linux-aarch64   # CI/runner 可稳定提供时列入正式支持；否则标 experimental，不得假装已验证
```

默认发布一个 Sigil plugin ZIP，内含共享 `data/` 与各正式支持平台的 native library。启动时只选择当前 platform triple 的一个 library；其他二进制不得被加载。

若未来因体积改成 platform-specific ZIP，版本、Python 代码、data hash 与行为必须相同，只允许 native binary 不同。

### 4.3.2 Native build 约束

- native library MUST 从 pinned `BYVoid/OpenCC` tag/commit 构建；
- 不允许修改 conversion source / dictionary semantics；
- MAY 对 install name、RPATH、静态/动态依赖布局进行仅打包性质的构建设置，但必须记录在 build recipe；
- release runner 必须执行 clean-environment load smoke test；
- bundled library 不得依赖插件未携带且 OS 不保证存在的第三方动态库；所有非系统 runtime dependencies 必须列入 manifest、随包携带并分别 hash；
- Windows 若存在同目录已验证依赖 DLL，loader MAY 在加载前使用 `os.add_dll_directory(native_dir)`，但只允许该 manifest 目录；
- macOS/Linux 应优先使用相对自身的 install-name/RPATH（如 `@loader_path` / `$ORIGIN`）解析已验证依赖；
- macOS release 必须在真实 Sigil release build 上验证 `ctypes` load；若签名/Library Validation 阻止加载，则该 release 不得发布，不能静默回退到系统 OpenCC；
- Windows 必须验证非 ASCII 用户路径；Linux 必须记录 glibc baseline。
...
## 4.5 Native Backend 事实约束（实现必须遵守）

### 4.5.1 Library 加载隔离

插件必须使用绝对路径加载 `vendor/MANIFEST.json` 指定的当前平台 library：

```python
ctypes.CDLL(absolute_verified_library_path)
```

禁止：

- `ctypes.util.find_library("opencc")`；
- `import opencc` 作为生产 backend；
- 调用用户 PATH 中的 `opencc`；
- 从 `/usr/lib`、Homebrew、MacPorts、Conda、系统 Python site-packages 自动寻找 OpenCC；
- bundled library 缺失时静默 fallback。

加载前计算 SHA-256；不匹配即 `DataIntegrityError`。加载后执行 C ABI symbol self-check；结果写入 session summary：
...
`ctypes` wrapper 必须显式声明 `argtypes/restype`，不得依赖 ctypes 默认推断。

转换：

```text
UTF-8 encode input
→ opencc_convert_utf8(handle, bytes, byte_length)
→ NULL => ConversionError + opencc_error()
→ ctypes.string_at(ptr)
→ UTF-8 decode
→ opencc_convert_utf8_free(ptr) in finally
```

规则：

- `opencc_convert_utf8_free()` 必须保证 exactly once；
- converter handle 必须在 session/backend close 时 `opencc_close()`；
- `opencc_open()` 返回 `(opencc_t)-1` 时视为失败；
- Windows/MSVC bundle 若导出 `opencc_open_w`，loader SHOULD 优先用宽字符 config path，保证非 ASCII 用户目录；否则使用 UTF-8 `opencc_open` 并由 Windows path smoke test 证明可用；
- `opencc_error()` 调用由 backend lock 保护，因为 upstream 明确其非线程安全；
- V1 默认一个 conversion worker；不得对同一个 handle 无证明地并发调用。

### 4.5.3 Config 与 dictionary 解析
...
调用必须给 `opencc_open()` **绝对 config path**。官方 Config loader 会使用 config parent directory 解析其引用的 dictionary，因此 V1 不需要修改 process cwd，也不需要设置全局 `OPENCC_DATA_DIR`。

禁止为了方便修改全进程 cwd；禁止在普通路径下永久修改 `OPENCC_DATA_DIR`。

### 4.5.4 Tofu-risk dictionary policy

稳定 C API `opencc_open()` 使用 OpenCC 默认 `ConfigLoadOptions`，在 pinned 1.4.2 中 `includeTofuRiskDictionaries=true`。
...
- canonical CLI golden 必须使用同一默认 policy。

未来若需要关闭该类 dictionary，必须升级为受控 C++ bridge/API，不能通过魔改官方 config 静默实现。

### 4.5.5 C API 不提供 dictionary 命中 trace

稳定 C API 只提供最终 converted string，不提供实际 segment/dictionary-hit trace。

因此：

- 插件不得调用/绑定 OpenCC 未列入稳定 C ABI 的 C++ 私有符号来获取命中；
- `rule_source` 不得伪造 `TWPhrases.txt`、`STCharacters` 等词典名；
- V1 对 `variant`/`regional` 的分类使用 §68 **比较配置差分（comparative config diff）**；
- 比较结果只是 UI/日志分类 heuristic，不代表 OpenCC 内部真实执行层。

### 4.5.6 normalization 是 canonical behavior

官方 config 的 normalization 属实际 OpenCC output：

- 插件不得预先删除或跳过；
- 若需要把某变化标记为 `normalization`，必须通过受控 diagnostic comparison config 得到证据；无法证明时分类为普通 OpenCC change，不得猜测；
- 结构不变量只保护**未计划 source span**，不会阻止 OpenCC 对 TextTarget 内兼容表意文字做合法 normalization。

### 4.5.7 Custom config sandbox

V1.1 专业模式若开放 custom config，不能把用户任意路径直接交给 native loader。
...
→ reject path traversal / unsupported plugin segmentation
→ copy approved config + dictionaries into cache/configs/<sha256>/
→ rewrite resource paths to sandbox-local relative paths if needed
→ hash snapshot
→ opencc_open(absolute_sandbox_config)
```

用户 custom config 与资源成为本次 `BackendProvenance` 的一部分。Preview 后 snapshot 不得变化。
...
以下样例是该规则模型的**必须回归用例**，实现后应使用 pinned native OpenCC 1.4.2 / `s2twp` 验证：
...
- native backend 的 tofu policy 不属于 Profile 参数，固定由 pinned C API default 决定并记录在 provenance；
...
```json
{
  ...
  "config": "s2twp",
  "segmentation": "mmseg",
  "backend_name": "OpenCC native C API",
  "opencc_version": "1.4.2",
  "opencc_upstream_tag": "ver.1.4.2",
  "opencc_upstream_commit": "...",
...
│       ├── opencc_backend/
│       │   ├── interface.py
│       │   ├── native.py              # high-level native backend
│       │   ├── ffi.py                 # ctypes C ABI declarations/ownership
│       │   ├── loader.py              # platform + hash verified loader
...
│           ├── MANIFEST.json
│           └── opencc/
│               ├── data/              # pinned official json + ocd2
│               │   ├── s2t.json
│               │   └── ...
│               └── native/
│                   ├── windows-x86_64/
...
`vendor/opencc/native/*` 是唯一允许运行的 OpenCC binary 来源；`vendor/opencc/data/` 是唯一默认 config/dictionary 来源。
...
<plugin>
  ...
  <type>edit</type>
  <author>...</author>
  <description>Professional Chinese conversion for EPUB using native OpenCC</description>
...
- V1 项目代码目标 Python >= 3.9（用于 typing/dataclasses/现代标准库），启动必须检查 `sys.version_info >= (3, 9)`；这是**本插件代码基线**，不再由第三方 Python OpenCC 包决定；
- `plugin.py` 只允许标准库 bootstrap + Controller 入口，不得直接加载 OpenCC library、解析 XHTML 或写业务逻辑；
- native OpenCC 由 `opencc_backend.loader` 在 runtime 通过 manifest 验证后加载，**不需要也禁止为 OpenCC 修改 `sys.path`**。
...
```text
NativeOpenCCBackend
  ├─ NativeLibraryLoader   # manifest / platform / sha256 / CDLL
  ├─ OpenCCFFI             # opencc.h stable C ABI
  └─ ConverterHandleCache  # one handle per config, session scoped
```
...
UI 不得直接调用 `ctypes`、`OpenCCFFI` 或持有 native handle。
...
# 29. Layer A：Canonical Native OpenCC Compatibility
...
```text
BYVoid/OpenCC native, pinned tag/commit
```

Golden 生成路径使用**由同一 pinned upstream source 构建的官方 CLI `opencc`**；插件生产路径使用 bundled `libopencc` + stable C ABI (`ctypes`)。
...
vs
plugin C-ABI wrapper invocation
```
...
2. `vendor/opencc/data/` 中存在；
3. hash 在 `vendor/MANIFEST.json` 中；
4. native backend 可以 `opencc_open()` 成功；
...
`tools/update_golden.py` 只能调用 pinned canonical CLI 生成 candidate expected，然后输出 diff 供人工 review；不得直接用 `NativeOpenCCBackend` 自己刷新 expected。
...
基线必须分别记录 `convert`（selected config final target）与 `classify`（§68 comparison configs + diff）的耗时；comparative classification 使 Analyze 阶段的 native OpenCC 调用通常最多为片段数 × 3。若 10 MB corpus 的 classify 耗时超过 convert 的 3 倍，视为实现问题（例如 difflib 未按字符序列使用）。
...
CI 分成 Python/plugin 与 native bundle 两类。
...
release/nightly 必须对每个正式支持 platform triple：

```text
build pinned OpenCC source
verify expected symbols
verify dynamic dependencies
verify native SHA-256 manifest
load via ctypes in clean process
load all 16 configs
run canonical smoke corpus
```

macOS 必须至少覆盖 arm64；Windows x64 必须覆盖非 ASCII temp/plugin path。

## 41.3 Golden

- ordinary PR 可使用仓库中已冻结的 canonical golden；
- release CI MUST 至少在一个平台重新构建 pinned official CLI 并执行 §75 differential；
- OpenCC version update PR MUST 在全部正式支持平台重新执行 native build + golden。

`tests/performance/` 只在 nightly/release 执行。GUI smoke test只在支持环境执行。

# 42. Build / Package

Release 分为 native artifact build 与 plugin package 两步。

## 42.1 Native artifact build

`native_build/build_opencc.py`：

1. checkout/download pinned BYVoid/OpenCC tag + verify commit；
2. build official `libopencc` 与官方 CLI；
3. 不修改 conversion source；
4. collect `data/config/*.json` 与 build 产生的 `.ocd2`；
5. run upstream tests；
6. run CLI golden；
7. inspect dynamic dependencies；
8. compute SHA-256；
9. produce platform artifact + metadata。
...
1. 清理 build；
2. 收集已通过 CI 的 native artifacts；
3. 验证所有 artifact tag/commit/version 一致；
4. 构建共享 `vendor/opencc/data/`；
5. 验证每个平台 native library 可以读取这份 shared data；
6. 生成 `vendor/MANIFEST.json`；
7. 运行 tests；
8. 检查 plugin.xml；
9. 解包自检；
10. 构建：
...
正式 release 不允许在用户机器上现场编译 OpenCC。

# 43. Dependency 供应链

发布包必须：

- bundled OpenCC native library 仅来自 pinned upstream source build；
- 保存 OpenCC Apache-2.0 LICENSE 与所有构建时纳入组件的第三方 notices；
- 每个 native/data 文件有 SHA-256；
- 构建 provenance 可追溯到 upstream tag/commit 与 CI run；
- 不在 runtime 联网下载 library/dictionary；
- 不执行系统中未验证的 OpenCC binary；
- 不允许 bundled library 存在未声明的第三方 runtime dependency。
...
native ABI/symbol check
dictionary/config diff summary
golden diff summary
regional changes
...
不得只复制新词典，也不得只替换某个平台 binary。

必须：

```text
pin new upstream tag + commit
↓
review upstream NEWS / config schema / C API
↓
build official CLI + libopencc on supported platforms
↓
run upstream tests
↓
generate candidate canonical golden using CLI
↓
run C-ABI wrapper differential
↓
ambiguity diff
↓
regional diff
...
manual review
↓
regenerate vendor/MANIFEST.json
↓
...
若 C ABI 符号发生 breaking change，或 golden diff 超过人工阈值，升级 PR 不得自动合并。
...
Native library manifest/hash: PASS
Native library load: PASS
C ABI symbols: PASS
...
Self-Test 不得加载系统 OpenCC；其 native path/hash 必须与当前 session backend 完全一致。
...
- pinned native OpenCC bundle、C ABI wrapper、manifest/hash loader（§4.1–4.5）；
- backend interface（§25.2，含 `comparison_configs`）；
- 全部 16 个 V1 allowlist config 可由 native `opencc_open()` 加载；
- native tofu policy 固定与 provenance；
- official CLI canonical golden 与 C-ABI differential（§29、§75）；
...
6. **Sigil 仅作为 adapter。**
7. **使用 pinned BYVoid/OpenCC native `libopencc`，通过官方稳定 C ABI + `ctypes` 调用；不使用 Python 重实现。**
8. **普通简繁与地区词汇严格分开。**
...
本文生产 backend 只基于 **BYVoid/OpenCC native**。Python 侧只使用标准库 `ctypes` 绑定稳定 C ABI。OpenCC CLI 只作为 canonical golden/oracle invocation 与开发诊断工具，不作为普通 TextTarget 的 runtime 转换路径。
...
- 不得调用未列入稳定 C ABI 的 C++ internal/private symbols；
- 不得让分类逻辑参与生成最终转换结果。
...
class BackendProvenance:
    backend_name: str                 # "OpenCC native C API"
    opencc_version: str               # "1.4.2"
    upstream_tag: str
    upstream_commit: str
    platform_triple: str
    native_library_path_id: str       # manifest logical id，不记录用户绝对路径到公开报告
...
# 75. Canonical CLI vs Plugin Native C-ABI Differential Test
...
vs
plugin `NativeOpenCCBackend` using bundled libopencc C ABI
```
...
PLATFORM_NATIVE_DIVERGENCE
UPSTREAM_BUILD_DIVERGENCE
```

不得以 `EXPECTED_BACKEND_LIMITATION` 把 native wrapper 与同源 CLI 的差异长期 xfail。
...
canonical CLI vs C-ABI differential = 100%
all 16 native configs load = PASS
native/data provenance checksum = PASS
```
...
2. **Native OpenCC only**：生产转换只允许 pinned BYVoid/OpenCC native library 的稳定 C ABI；不得用 Python 重实现或系统 OpenCC fallback。
3. **OpenCC provenance**：upstream tag/commit、platform、native library hash、data/config hash、tofu policy 一起冻结记录。
4. **Canonical golden compatibility**：插件 C-ABI output 与同源 pinned official CLI oracle 逐字一致。
...
10. **Failure means no partial EPUB mutation**：任何 verify/native/FFI 错误 → 返回非 0，Sigil 丢弃全部修改。
11. **Core/Sigil/UI separation**：只有 adapter commit stage 写书；UI 不持有 native handle；backend 不 import UI/Sigil。
12. **只用 OpenCC 稳定 C ABI**：不得绑定 C++ internal/private symbol 获取词典命中；`rule_source` 不写未经证实的词典文件名。
13. **Verified native loader**：只从 `vendor/MANIFEST.json` 绝对路径加载，先 hash 再 `ctypes.CDLL`；禁止 `find_library`/PATH/Homebrew/system fallback。
